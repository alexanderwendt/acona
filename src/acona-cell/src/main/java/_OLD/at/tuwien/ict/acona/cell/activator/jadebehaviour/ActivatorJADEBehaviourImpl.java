package _OLD.at.tuwien.ict.acona.cell.activator.jadebehaviour;
...
public class ActivatorJADEBehaviourImpl implements Activator {

	protected static Logger log = LoggerFactory.getLogger(ActivatorJADEBehaviourImpl.class);
	
	private String name;
	private CellFunction behavior;
	private Cell caller;
	...
	@Override
	public Activator initWithConditions(String name, Map<String, List<Condition>> subscriptionCondition, String logic, CellFunction behavior, Cell caller) {
		this.name = name;
	...
	public boolean runActivation(Datapoint subscribedData) {
		...
		if (this.conditionCurrentCount==this.conditionMaxCount) {
			...
			behavior.updateData(currentDatapointList);	//Add data that shall be used in the behavior
