...
@Deprecated
public class _OLD_SubscribeDataServiceBehavior extends SimpleAchieveREResponder {
	...
	public _OLD_SubscribeDataServiceBehavior(CellImpl caller) {
		//In the super class, it shall use the message template here
		super(caller, MessageTemplate.and(MessageTemplate.MatchProtocol(FIPANames.InteractionProtocol.FIPA_SUBSCRIBE), MessageTemplate.MatchOntology(AconaServiceType.SUBSCRIBE.toString())));
		this.cell = caller;
		log.debug("Responder ready. Waiting for incoming SUBSCRIBE request");
	}
	...
	public ACLMessage prepareResponse(ACLMessage request) {
		...
		try { 
			...
			log.info("OK to subscribe.");
		
		} catch (Exception fe){
			log.error("Error handling the SUBSCRIBE action.", fe);
	...
	public ACLMessage prepareResultNotification(ACLMessage request, ACLMessage response) {
		...
		try {
			...
			this.datapointList.forEach(dp->{
				this.cell.getDataStorage().subscribeDatapoint(dp.getAddress(), msg.getSender().getLocalName());
			...
			log.error("Cannot process request SUBSCRIBE", e);
