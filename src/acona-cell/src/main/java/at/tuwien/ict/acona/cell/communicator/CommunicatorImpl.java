...
public class CommunicatorImpl extends AgentCommunicatorImpl implements BasicServiceCommunicator {
	...
	public ServiceState executeServiceBlocking(String agentAndService, int timeout) {
		String servicecommand = agentAndService + "." + CellFunctionThreadImpl.COMMANDSUFFIX;
		String serviceresult = agentAndService + "." + CellFunctionThreadImpl.STATESUFFIX;

		Datapoint command = DatapointBuilder.newDatapoint(servicecommand);
		Datapoint state = DatapointBuilder.newDatapoint(serviceresult);

		ServiceState result = ServiceState.ERROR;
		try {
			Datapoint dp = this.queryDatapoints(command.getAgent(), command.getAddress(), new JsonPrimitive(ControlCommand.START.toString()), state.getAgent(), state.getAddress(), new JsonPrimitive(ServiceState.FINISHED.toString()), timeout);
	...
	public ServiceState executeServiceBlocking(String agentAndService) {
		return this.executeServiceBlocking(agentAndService, this.getDefaultTimeout());
	}
