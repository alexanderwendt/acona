...
public class CellBuilder {
	...
	// private static final String CELLFUNCTIONMAP = "cellfunctionmap"; //id to
	// datapoint to subscribe
	...
	// private final Map<String, Condition> conditionMap = new
	// ConcurrentHashMap<String, Condition>();
	// private final Map<String, CellFunction> cellFunctionBehaviourMap = new
	// ConcurrentHashMap<String, CellFunction>();
	// private final Map<String, CellFunction> activatorMap = new
	// ConcurrentHashMap<String, CellFunction>();
	...
	public void initializeCellConfig(CellConfig config, CellInitialization caller) throws Exception {
		try {
			...
			// JsonArray cellFunctions =
			// config.getAsJsonArray(CellConfig.CELLFUNCTIONS);
	...
	// private void initializeCellConfigCellFunctions(JsonObject config,
	// CellInitialization caller) throws Exception {
	// try {
	// //=== Instantiate the conditions ===
	// //Get all condition configs
	...
	// try {
	// Activator activator =
	// this.createCellFunctionFromConfig(activatorConfig.getAsJsonObject(),
	// caller);
	// if (activator==null) {
	// throw new NullPointerException("activator does not exist");
	// }
	...
	private synchronized CellFunction createCellFunctionFromConfig(JsonObject config, Cell caller) throws Exception {
	...
	// private CellFunction createActivatorFromConfig(JsonObject config, Cell
	...
	// JsonArray conditionMapping = config.get(ACTIVATORMAP).getAsJsonArray();
	// //"key : "[value, value, value]"
	// String activatorLogic =
	// (config.get(ACTIVATORLOGIC)!=null?config.get(ACTIVATORLOGIC).getAsString():"");
	//
	//
	// try {
	// //Get behaviour
	// CellFunction cellFunctionBehaviour =
	// cellFunctionBehaviourMap.get(behaviourName);
	//
	// if (cellFunctionBehaviour==null) {
	// throw new NullPointerException("No behaviour with name " + behaviourName
	// + " found in map " + cellFunctionBehaviourMap + ".");
	// }
	//
	// //Create condition address map
	// final Map<String, List<Condition>> datapointConditionMap = new
	// ConcurrentHashMap<String, List<Condition>>();
	// conditionMapping.forEach(mapping->{
	// final Set<Entry<String, JsonElement>> set =
	// mapping.getAsJsonObject().entrySet();
	// set.forEach(e->{
	// //Get datapointname
	...
	// }
	//
	// result = new ActivatorJADEBehaviourImpl().initWithConditions(name,
	// datapointConditionMap, activatorLogic, cellFunctionBehaviour, caller);
	//
	// } catch (Exception e) {
	...
	// private void initializeCellConfigJADEBehaviour(JsonObject config,
	// CellInitialization caller) throws Exception {
	...
	// //If there is an activator, then there must be conditions and behaviour.
	// If there is no activator, then it is an empty configuration
	//
	// if (cellActivators==null) {
	// log.debug("There are no activators for this cell. Therefore, it is
	// empty");
	// } else {
	// if (cellActivators!=null && (cellBehaviours==null ||
	// cellConditions==null)) {
	// throw new Exception("There is an activator, but either no conditions or
	// behaviours. Both are needed to create an activator");
	// }
	//
	//
	...
	// try {
	// Condition c =
	// this.createConditionFromConfig(conditionConfig.getAsJsonObject());
	...
	// try {
	// CellFunction behaviour =
	// this.createCellFunctionBehaviourFromConfig(behaviourConfig.getAsJsonObject(),
	// caller);
	// this.cellFunctionBehaviourMap.put(behaviour.getFunctionName(),
	// behaviour);
	// } catch (Exception e) {
	...
	// try {
	// Activator activator =
	// this.createActivatorFromConfig(activatorConfig.getAsJsonObject(),
	// this.conditionMap, this.cellFunctionBehaviourMap, caller);
	// if (activator==null) {
	// throw new NullPointerException("activator does not exist");
	// }
	// this.activatorMap.put(activator.getActivatorName(), activator);
	...
	// private Condition createConditionFromConfig(JsonObject config) throws
	// Exception {
	...
	// config.get(CONDITIONCONFIG).getAsJsonObject();
	//
	// //Generate the class
	// try {
	//
	...
	// throw new NoSuchMethodException(e.getMessage());
	// } catch (SecurityException e) {
	// log.error("Security exception for constructor for class {}", className,
	// e);
	// throw new SecurityException(e.getMessage());
	// } catch (IllegalArgumentException e) {
	// log.error("Wrong arguments in constrctor of class {}", className, e);
	...
	// private CellFunction createCellFunctionBehaviourFromConfig(JsonObject
	// config, Cell caller) throws Exception {
	// CellFunction result = null;
	//
	// //Get all values
	// String name = config.get(BEHAVIOURNAME).getAsString();
	// String className = config.get(BEHAVIOURCLASS).getAsString();
	// //JsonObject behaviourConfig =
	// config.get(BEHAVIOURCONFIG).getAsJsonObject();
	//
	// //Generate the class
	//
	// try {
	...
	// private Activator createActivatorFromConfig(JsonObject config,
	// Map<String, Condition> conditionMap, Map<String, CellFunction>
	// cellFunctionBehaviourMap, Cell caller) throws Exception {
