...
public class CellFunctionHandlerImpl implements CellFunctionHandler {
	...
	@Override
	public synchronized void activateLocalFunctions(Datapoint subscribedData) {
		...
		synchronized (this) {
			...
			activatorAddresses.forEach(subscriptionsConfig -> {
				...
				try {
					...
					log.error("Cannot unsubscribe address={}", subscriptionsConfig.getAddress());
				}
	...
	public String toString() {
		...
		builder.append(cellFunctionMap.values());
		return builder.toString();
	}

	@Override
	public Map<String, List<CellFunction>> getCellFunctionMapping() {
		return Collections.unmodifiableMap(cellFunctionMap);
	}
