		...
		try {
			...
			CellConfig controllerAgentConfig = CellConfig.newConfig(controllerAgentName)
					.addCellfunction(CellFunctionConfig.newConfig(SequenceController.class)
						...
						.addSyncDatapoint(DatapointConfig.newConfig(COMMANDDATAPOINTNAME, COMMANDDATAPOINTNAME, "", "push")));
			...
			//controller.subscribeForeignDatapoint(processDatapoint, memoryAgentName);
			...
			CellConfig serviceAgent1 = CellConfig.newConfig(agentName1)
					.addCellfunction(CellFunctionConfig.newConfig(ServiceName, CFIncrementService.class)
							.addSubscription(DatapointConfig.newConfig(INCREMENTATIONDATAPOINTNAME, processDatapoint, memoryAgentName)));
			...
			Datapoint state = controller.getCommunicator().query(Datapoint.newDatapoint(COMMANDDATAPOINTNAME).setValue(ControlCommand.START.toString()), controller.getCell().getLocalName(), Datapoint.newDatapoint("state"), controller.getCell().getLocalName(), 10000);
		...
		try {
			...
			CellConfig controllerAgentConfig = CellConfig.newConfig(controllerAgentName)
					.addCellfunction(CellFunctionConfig.newConfig(SequenceController.class)
							...
							.setProperty("agent3", agentName3)
							.setProperty("servicename", ServiceName)
							.addSubscription(DatapointConfig.newConfig(COMMANDDATAPOINTNAME, COMMANDDATAPOINTNAME)));
			CellGatewayImpl controller = this.launcher.createAgent(controllerAgentConfig);
			...
			CellConfig serviceAgent1 = CellConfig.newConfig(agentName1)
					.addCellfunction(CellFunctionConfig.newConfig(ServiceName, CFIncrementService.class)
							.addSyncDatapoint(INCREMENTATIONDATAPOINTNAME, processDatapoint, memoryAgentName));
			
			
			CellConfig serviceAgent1 = CellConfig.newConfig(agentName1)
					.addCellfunction(ServiceName, CFIncrementService.class)
							.addSyncDatapoint("rawdata", AGENTENNAME));
			
			CellGatewayImpl service1 = this.launcher.createAgent(serviceAgent1);
			
			CellConfig serviceAgent2 = CellConfig.newConfig(agentName2)
					.addCellfunction(CellFunctionConfig.newConfig(ServiceName, CFIncrementService.class)
							.addSyncDatapoint(DatapointConfig.newConfig(INCREMENTATIONDATAPOINTNAME, processDatapoint, memoryAgentName)));
			CellGatewayImpl service2 = this.launcher.createAgent(serviceAgent2);
