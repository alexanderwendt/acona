...
public class AconaServiceTester {
	...
	public void externalControllerWithDatabaseCellsAndAdditionCellTest() {
		try {
			...
			controller.writeLocalDatapoint(Datapoint.newDatapoint(COMMANDDATAPOINTNAME).setValue(ControlCommand.START.toString()));
...
//			double sum = controlAgent.getCommunicator().read(resultdatapoint, outputmemoryAgentName).getValue().getAsJsonPrimitive().getAsDouble();
			...
			log.debug("correct value={}, actual value={}", expectedResult, 0.1);
			
			assertEquals(0.1, expectedResult, 0.0);
