...
public class ReadOperandBehaviour extends CellFunctionBehaviourImpl {
	...
	public void function(Map<String, Datapoint> data) {
		//Send first message
		Behaviour sendBehaviour1 = new SendDatapointOnDemandBehavior(new AID(conf.get(OPERAND1AGENTNAME).getAsString(), AID.ISLOCALNAME), Datapoint.newDatapoint(conf.get(OPERAND1ADDRESS).getAsString()), AconaService.READ);
		...
		//Send second message
		Behaviour sendBehaviour2 = new SendDatapointOnDemandBehavior(new AID(conf.get(OPERAND2AGENTNAME).getAsString(), AID.ISLOCALNAME), Datapoint.newDatapoint(conf.get(OPERAND2ADDRESS).getAsString()), AconaService.READ);
		...
		//ACLMessage msg = ACLUtils.convertToACL(Message.newMessage().addReceiver(conf.get(OPERAND2AGENTNAME).toString()).setContent(Datapoint.newDatapoint(conf.get(OPERAND2ADDRESS).getAsString())).setService(AconaService.READ));
