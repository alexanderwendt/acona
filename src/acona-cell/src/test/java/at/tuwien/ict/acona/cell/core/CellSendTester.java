...
public class CellSendTester {
	...
	public void queryControllerWithForeignDatapointTest() {
		try {
			...
			CellConfig testagent = CellConfig.newConfig(serviceAgentName, CellImpl.class)
					.addCellfunction(CellFunctionConfig.newConfig("testExecutor", CFDurationThreadTester.class)
							.addSubscription(DatapointConfig.newConfig("command", commandDatapoint, "subscriptionagent"))
							.addSubscription(DatapointConfig.newConfig("query", queryDatapoint, "subscriptionagent"))
							.addSubscription(DatapointConfig.newConfig("executeonce", executeonceDatapoint, "subscriptionagent"))
							.setProperty("result", resultDatapoint));
			this.launchUtil.createAgent(testagent);
			...
			String result = cellControlSubscriber.getCommunicator().query(Datapoint.newDatapoint(queryDatapoint).setValue("SELECT * FILESERVER"), "subscriptionagent", Datapoint.newDatapoint(resultDatapoint), serviceAgentName, 100000).getValueAsString();
			log.debug("Received back from query={}", result);
