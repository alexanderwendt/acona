...
public class CellSendTester {
	...
	public void singleReadtest() {
		try {
			...
			this.comm.sendAsynchronousMessageToAgent(Message.newMessage().addReceiver("dbagent1").setContent(Datapoint.newDatapoint(operand1address).setValue(new JsonPrimitive(11.))).setService(AconaService.WRITE));
			
			//Trigger the calculator agent
			this.comm.sendAsynchronousMessageToAgent(Message.newMessage().addReceiver("AdditionAgent").setContent(Datapoint.newDatapoint(triggeraddress).setValue(new JsonPrimitive("START"))).setService(AconaService.WRITE));
	...
	public void asynchronousReadtest() {
		try {
			...
			//Write the numbers in the database agents
			this.comm.sendSynchronousMessageToAgent(Message.newMessage().addReceiver("dbagent1").setContent(Datapoint.newDatapoint(operand1address).setValue(new JsonPrimitive(11.))).setService(AconaService.WRITE));
			this.comm.sendAsynchronousMessageToAgent(Message.newMessage().addReceiver("dbagent2").setContent(Datapoint.newDatapoint(operand2address).setValue(new JsonPrimitive(22.))).setService(AconaService.WRITE));
			this.comm.sendAsynchronousMessageToAgent(Message.newMessage().addReceiver("AdditionAgent").setContent(Datapoint.newDatapoint("s3.receiveragent").setValue(new JsonPrimitive("receiveragent"))).setService(AconaService.WRITE));
