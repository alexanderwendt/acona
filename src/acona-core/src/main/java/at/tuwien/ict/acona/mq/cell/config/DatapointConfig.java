...
public class DatapointConfig {
	...
	public static DatapointConfig newConfig(String name, String address, SyncMode syncmode) {
		at.tuwien.ict.acona.mq.datastructures.Datapoint dp = (new DPBuilder()).newDatapoint(address);
	...
	/**
	 * Return the destination agentid. Compare the local agent id with the destination id. If the default value "" is used, it means that the destination should be the local agent
	 * 
	...
	public Datapoint toDatapoint(String localAgentName) {
		return (new DPBuilder()).newDatapoint(this.getAgentid(localAgentName) + ":" + this.getAddress());
	}
