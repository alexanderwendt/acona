...
import at.tuwien.ict.acona.mq.cell.core.CellFunctionHandlerListener;
 ...
 *
 */
public class StateMonitor extends CellFunctionThreadImpl implements CellFunctionHandlerListener {
	...
	private void generateSystemState() throws Exception {
		...
		Chunk systemState = ChunkBuilder.newChunk("SystemState", "SYSTEMSTATE")
				.setValue("agentname", this.getCellName())
	...
	private void initializeFunction(String functionRootAddress) throws Exception {
		...
		String description = this.getCommunicator().read(functionRootAddress + "." + CellFunctionThreadImpl.DESCRIPTIONSUFFIX).getValueOrDefault(new JsonPrimitive("No description available.")).getAsString();
