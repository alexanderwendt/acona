	...
	protected void cellFunctionThreadInit() throws Exception {
		try {
			...
			try {
				...
				Response response = this.getCommunicator().execute(this.codeletHandlerAddress + "/" + CellFunctionCodeletHandler.REGISTERCODELETSERVICENAME, request, this.timeout);
...
//	}
	...
	private void updateServiceStateInCodeletHandler(ServiceState state) throws Exception {
		...
		Response response = this.getCommunicator().execute(this.codeletHandlerAddress + "/" + CellFunctionCodeletHandler.SETSTATESERVICENAME, request, this.timeout);
	...
	protected void shutDownThreadExecutor() throws Exception {
		...
		Response response = this.getCommunicator().execute(this.codeletHandlerAddress + "/" + CellFunctionCodeletHandler.UNREGISTERCODELETSERVICENAME, request, this.timeout);
		if (response.hasError()) {
			throw new Exception("Communication error. Error: " + response.getError());
		}
	}
